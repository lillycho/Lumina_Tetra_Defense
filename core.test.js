...
test("게임은 CC0 전역 클릭 효과음만 연결하고 배경음은 쓰지 않는다", () => {
  ...
  assert.equal("bgm" in ART_ASSETS.audio, false);
  assert.doesNotMatch(gameJs, /backgroundMusic/);
  assert.doesNotMatch(gameJs, /unlockAudio/);
  assert.doesNotMatch(gameJs, /ART_ASSETS\.audio\.bgm/);
